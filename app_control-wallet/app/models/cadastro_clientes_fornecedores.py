...
class CadastroClientesFornecedores(db.Model, SerializerMixin):
    ...
    cep = db.Column(db.Numeric(8), unique=True, nullable=False)
    endereco_rua = db.Column(db.String(), nullable=False)
    endereco_numero = db.Column(db.Numeric(10), unique=True, nullable=False)
    cidade = db.Column(db.String(), nullable=False)
    estado = db.Column(db.String(), nullable=False)
    telefone = db.Column(db.Numeric(10), unique=True, nullable=False)
    celular = db.Column(db.Numeric(11), unique=True, nullable=False)
